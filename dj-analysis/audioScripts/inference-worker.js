...
let modelURL, musiCNN, modelName;
const modelTagOrder = {
  ...
  'label': ["rock", "pop", "alternative", "indie", "electronic",
  ...
  'bpm': Array.from({ length: 256 }, (_, i) => i + 30)
};
const modelImportantIndex = {
  'mood_happy': 0,
  'mood_sad': 1,
  'mood_relaxed': 1,
  'mood_aggressive': 0,
...
self.onmessage = e => {
  if (typeof e.data === 'string' || e.data instanceof String) {
    // initialize
    modelURL = `../audioModels/${e.data}/model.json`;
    modelName = e.data;
    musiCNN = new EssentiaModel.TensorflowMusiCNN(tf, modelURL);

    musiCNN.initialize().then(() => {
      console.log(`${modelName}: essentia-tfjs model ready...`);
      warmUp();
    });
    ...
    musiCNN.predict(e.data, true)
      .then((predictions) => {
        let l = predictions[0].length;
        let div = predictions.length;
        console.log('predictions: ', predictions);
        let avgPredictions = predictions
          .reduce((acc, val) => {
            return acc.map((x, i) => x + val[i]);
          }, new Array(l).fill(0))
          .map(x => x / div);


        let index = modelImportantIndex[modelName];
        if (index === 'max') {
          self.postMessage(getMax(avgPredictions, modelName));
        } else if (index === 'all') {
          self.postMessage(getObject(avgPredictions, modelName));
        } else {
          self.postMessage(avgPredictions[index]);
        }

        // let finalPrediction = [];
        // for (let i = 0; i < modelTagOrder[modelName].length; i++) {
        //   let currentTag = modelTagOrder[modelName][i];
        //   if (currentTag) {
        //     let currentPrediction = currentTag + ': ' + avgPredictions[i];
        //     finalPrediction.push(currentPrediction);
        //   }
        // }
...
function warmUp() {
  ...
  musiCNN.predict(fakeFeatures, false).then(() => {
    console.info(`${modelName}: Warmed up inference`);
  });
}
...
function getMax(predictions, modelName) {
  let index = predictions.reduce((acc, val, index, arr) => {
    if (val > arr[acc]) return index;
    return acc;
  }, 0);
  return modelTagOrder[modelName][index];
}

function getObject(predictions, modelName) {
  let obj = {};
  for (let i = 0; i < predictions.length; i++) {
    obj[modelTagOrder[modelName][i]] = predictions[i];
  }
  return obj;
}